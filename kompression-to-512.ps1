...
param (
    ...
    [int]$maxSize = 0,                  # Maximum dimension for resizing (0 to skip resizing)
...
if (($quality -lt 0) -and ($maxSize -eq 0)) {
    Write-Host "No operations specified. Please specify either quality, maxSize, or both."
    Write-Host "Use -quality [0-100] to enable compression (or -1 to skip)"
    Write-Host "Use -maxSize [pixels] to enable resizing (or 0 to skip)"
    exit 1
}
...
Write-Host "Operations: $(if ($maxSize -gt 0) { "Resize to max ${maxSize}px" } else { "No resize" })$(if ($quality -ge 0) { " | Compress to quality $quality" } else { " | No compression" })"
        ...
        try {
            ...
            if ($width -gt $maxSize -or $height -gt $maxSize) {
                if ($maintainAspectRatio) {
                    ...
                    # Just cap the dimensions at maxSize
                ...
                $image.Dispose()
                Write-Host "  Image already smaller than ${maxSize}px, skipping resize"
            }
        }
        catch {
